...
public class AmazonSearchService {
    ...
    public static WebDriver getSearchResultPage(WebDriver webDriver, String keyWord) {
        ...
        TimeoutUtil.waitSeconds(2);
        return webDriver;
    }

    public static WebDriver goToNextPage(WebDriver webDriver) {
        TimeoutUtil.waitSeconds(3);
        WebElement nextButton = webDriver.findElement(By.className("a-pagination")).findElement(By.className("a-last"));
        nextButton.click();
        TimeoutUtil.waitSeconds(3);
        String currentUrl = webDriver.getCurrentUrl();
        webDriver.get(currentUrl);
